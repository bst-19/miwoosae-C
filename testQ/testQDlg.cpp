...
#define SIMPLE_BASE_VALUE 200
#define OFF_ON_BETWEEN_TIME 5000
...
void CtestQDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CROP2, m_btn_nocrop);
	DDX_Control(pDX, IDC_STATIC_IR4, m_txt_etc);
	DDX_Control(pDX, IDC_CHECK1, m_chbx_etc);
}
...
BEGIN_MESSAGE_MAP(CtestQDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_CROP2, &CtestQDlg::OnBnClickedCrop2)
END_MESSAGE_MAP()
		...
		if (startYn == 1) {
			...
			if (diff > SIMPLE_BASE_VALUE) {
				...
				if (changeTag == 2) {
					...
					wchar_t wtext[100];
					wchar_t second[10];
					wchar_t tmp_result[100];
					itoa(repeatNow + 1, cnt, 10);
					mbstowcs(wtext, cnt, strlen(cnt) + 1);
					wcscat(wtext, L" 번째 부트 타임: ");
					

					double bootTime = (double)CWatch.mTimeCheck();
					sprintf(buf, "%.3f", (bootTime / 1000));
					mbstowcs(second, buf, strlen(buf) + 1);

					wcscat(wtext, second);
					wcscat(wtext, L"초");

					wcscpy(tmp_result, second);
					wcscat(tmp_result, L"초");

					setLog(wtext);
					wcscpy(result[repeatNow], tmp_result);

					changeTag++;
					//setLog(L"쓰레드 실행 전");

					repeatNow++;
					if (repeatNow == repeatCnt) {
						int tmpNum = repeatCnt;
						StopFunc(2);
						//AfxMessageBox(_T("TEST"), MB_YESNO);
						
						KillTimer(1000);
						if (AfxMessageBox(_T("부트 타임 결과값을 엑셀로 추출 하시겠습니까?"), MB_YESNO | MB_ICONQUESTION) == IDYES) {
							makeExcel(tmpNum);
						}
			...
			if (changeTag == 1 && diff < SIMPLE_BASE_VALUE) {
				changeTag++;
			}
...
void CtestQDlg::BtnOffFunc(int param) {
	if (param == 0) {
...
void CtestQDlg::BtnOnFunc(int param) {
	if (param == 0) {
...
void CtestQDlg::setResources(void)
{
	...
	m_txt_repeat.MoveWindow(WINDOW_MARGIN + 10, WINDOW_MARGIN * 2.2, 54, 20);
	m_repeat_combo.MoveWindow(WINDOW_MARGIN + 55, WINDOW_MARGIN * 2.1, 65, 20);
	...
	m_repeat_combo.SetCurSel(0);
	m_btn_start.MoveWindow(WINDOW_MARGIN + 120, WINDOW_MARGIN + 300 + 10, 85, 50);
	m_btn_stop.MoveWindow(WINDOW_MARGIN + 210, WINDOW_MARGIN + 300 + 10, 58, 50);

	m_txt_IR.MoveWindow(WINDOW_MARGIN + 22, WINDOW_MARGIN * 4 - 10, 100, 20);
	m_btn_on.MoveWindow(WINDOW_MARGIN + 60, WINDOW_MARGIN * 3.5, 55, 25);
	m_btn_off.MoveWindow(WINDOW_MARGIN + 120, WINDOW_MARGIN * 3.5, 55, 25);
	
	m_txt_etc.MoveWindow(WINDOW_MARGIN + 190, WINDOW_MARGIN * 4 - 7, 50, 20);
	m_chbx_etc.MoveWindow(WINDOW_MARGIN + 240, WINDOW_MARGIN * 4 - 9, 27, 20);
	...
	m_btn_crop.MoveWindow(WINDOW_MARGIN + 135, WINDOW_MARGIN * 1.9, 65, 32);
	m_btn_nocrop.MoveWindow(WINDOW_MARGIN + 205, WINDOW_MARGIN * 1.9, 65, 32);
...
{
	repeatCnt = m_repeat_combo.GetCurSel() + 1;

	StartFunc(0);
}

void CtestQDlg::StartFunc(int param) {
	...
	if (param == 0) {
		...
		repeatCnt = m_repeat_combo.GetCurSel() + 1;
		itoa(repeatCnt, buf, 10);

		if (repeatCnt == 1) {
			setLog(L"1회 부트 타임 실행");
		}
		else {
			mbstowcs(wtext, buf, strlen(buf) + 1);
			wcscat(wtext, L"회 부트 타임 반복 실행");
			setLog(wtext);
			setLog(L"1번째 부트 타임 측정");
		}
	}else{
		char buf[10];
		wchar_t wtext[100];

		itoa(repeatNow + 1, buf, 10);

		mbstowcs(wtext, buf, strlen(buf) + 1);
		wcscat(wtext, L"번째 부트 타임 측정");
		setLog(wtext);
	}
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	bootTimeAval();
...
void CtestQDlg::StopFunc(int param) {
	
	if (param == 0){
		char buf[10];
		wchar_t wtext[100];

		itoa(m_repeat_combo.GetCurSel() + 1, buf, 10);
		mbstowcs(wtext, buf, strlen(buf) + 1);
		wcscat(wtext, L"번째 부트 타임 중간 취소");
		setLog(wtext);

		repeatCnt = 0;
		repeatNow = 0;
	}else if(param == 2){
		setLog(L"부트 타임 프로세스 완료");
		repeatCnt = 0;
		repeatNow = 0;	
	}
	...
	file_name = 0;
	changeTag = 0;
	//setLog(intToWchar(file_name));
	//setLog(L"종료 조건 완료");
	
}

UINT CtestQDlg::ThreadFirst(LPVOID _mothod) {
	CtestQDlg *fir = (CtestQDlg *)_mothod;
	CtestQDlg *pdlg = (CtestQDlg *)AfxGetApp()->m_pMainWnd;
	
	pdlg->BtnOffFunc(1);
	pdlg->StopFunc(1);
	pdlg->m_btn_start.EnableWindow(false);
	pdlg->m_btn_stop.EnableWindow(false);

	///SLEEP
	Sleep(OFF_ON_BETWEEN_TIME);
	///SLEEP

	pdlg->m_btn_stop.EnableWindow(true);
	pdlg->StartFunc(1);

	return 0;
}

wchar_t* CtestQDlg::intToWchar(int num) {
	char buf[10];
	wchar_t wtext[100];
	itoa(num, buf, 10);
...
void CtestQDlg::makeExcel(int num) {
	BasicExcel xls;
	xls.New(1);
...
// 엑셀에 행을 추가하기 위해 buffer에 기록
	String test = "TEST";
	
	int col, row = 0;
	wchar_t wtext[20];

	BasicExcelCell* title1 = sheet->Cell(0, 0);
	title1->Set(L"부트 타임 측정");
	BasicExcelCell* title2 = sheet->Cell(0, 1);
	title2->Set(L"측정 시간");

	for (col = 0; col < num; col++) {
		BasicExcelCell* cell = sheet->Cell(col + 1, 0);
		BasicExcelCell* cell_result = sheet->Cell(col + 1, 1);
		
		swprintf_s(wtext, L"%d", col+1);
		wcscat(wtext, L" 번째");

		cell->Set(wtext);
		cell_result->Set(result[col]);
	}
	//sprintf(buffer, "%ws", test.str().c_str());
	...
	//WinExec("test.xls", SW_SHOW);
	ShellExecute(NULL, L"open", _T("test.xls"), NULL, NULL, SW_SHOW);
	/*
	try {
		CFile file(L"test.xls", CFile::modeReadWrite);
	}
...
void CtestQDlg::OnBnClickedCrop2()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	setLog(L"이미지 Crop 취소");
	cropYn = 0;
}
